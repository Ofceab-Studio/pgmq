...
| conditional | jsonb    | Filters the messages by their json content. Defaults to '{}'::jsonb - no filtering. Supports two formats:<br><br>1. **Comparison operators**: `{"field": "VALUE", "operator": ">", "value": "2"}` - Supports operators: `>`, `<`, `>=`, `<=`, `=`, `!=`<br>2. **Exact match (backward compatible)**: `{"hello": "world_1"}` - Uses JSONB containment operator `@>` |
...
Read a message from a queue with exact match filtering (backward compatible)
...
```sql
-- Read messages where VALUE > 2
select * from pgmq.read('my_queue', 10, 10, '{"field": "VALUE", "operator": ">", "value": "2"}');
 msg_id | read_ct |          enqueued_at          |              vt               |       message        | headers
--------+---------+-------------------------------+-------------------------------+----------------------+---------
      2 |       1 | 2023-10-28 19:14:47.356595-05 | 2023-10-28 19:17:08.608974-05 | {"VALUE": 4}         |
      3 |       1 | 2023-10-28 19:14:47.356595-05 | 2023-10-28 19:17:08.608975-05 | {"VALUE": 10}        |
(2 rows)

-- Read messages where VALUE >= 4
select * from pgmq.read('my_queue', 10, 10, '{"field": "VALUE", "operator": ">=", "value": "4"}');
...
| conditional | jsonb    | Filters the messages by their json content using JSONB containment (`@>`). Defaults to '{}' - no filtering. **Note:** Currently only supports exact match format (e.g., `{"hello": "world_1"}`). Comparison operators are not yet supported in `read_with_poll()`. |
...
select * from pgmq.metrics_all();
      queue_name      | queue_length | newest_msg_age_sec | oldest_msg_age_sec | total_messages |          scrape_time          | queue_visible_length
----------------------+--------------+--------------------+--------------------+----------------+-------------------------------+---------------------
 my_queue             |           16 |               2563 |               2565 |             35 | 2023-10-28 20:25:07.016413-05 |                  12
 my_partitioned_queue |            1 |                 11 |                 11 |              1 | 2023-10-28 20:25:07.016413-05 |                   1
 my_unlogged          |            1 |                  3 |                  3 |              1 | 2023-10-28 20:25:07.016413-05 |                   1
```
...
| :---        |    :----   |          :--- |
| queue_name      | text       | The name of the queue   |
| throttle_interval_ms      | integer       | Minimum milliseconds between notifications. Defaults to 250. Set to 0 for no throttling (notify on every insert).   |
